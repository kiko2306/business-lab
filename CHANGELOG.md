...
- No error toasts while the backend restarts during an update: the status poll and resource strip no longer toast their own failures
...
- Outline (team wiki) as a new app, signing in through Authelia OIDC
...
- Mobile-app badges now use real Android/Apple glyphs instead of emoji
...
- "Claude API key" generalized into multi-provider "AI API Keys" (Anthropic, Google Gemini, Groq)

## [0.146.0] — 2026-09-24

### Added

- Social-drafts publish path: sends the draft as an email advert to every active subscriber

## [0.145.0] — 2026-09-24

### Added

- Self-update panel: richer progress detail — which image is building, which app is being recreated, and a real visible checking phase before a slow git fetch

## [0.144.0] — 2026-09-24

### Hotel

- Add the one-time legacy data import (apps/hotel/import/): check-in history and feedback responses, matched by Wintouch code and exact question text
...
- Hotel: check-in and post-stay quiz guest emails send on a per-unit schedule

## [0.140.1] — 2026-09-24

### Apps

- Group hotel and tally under a new "Wintouch Interop" dashboard category

## [0.140.0] — 2026-09-24

### Added

- Hotel's pulse app gets a guest frontend and public secondary exposure
...
- Hotel app: hotel-core with the shared schema, agent enrolment and per-unit access (plan.md §638)
...
- Translate the shared components (service-card, resource-strip, toast-container, confirm-dialog) to English/pt-PT

## [0.132.7] — 2026-09-23

### Multi-language UI

- Translate the Content, Utils, Access-denied and Set-password pages to English/pt-PT

## [0.132.6] — 2026-09-23

### Multi-language UI

- Translate the Settings, Self-update, Recovery and Setup pages to English/pt-PT

## [0.132.5] — 2026-09-23

### Multi-language UI

- Translate the Users, Account and Audit logs pages to English/pt-PT

## [0.132.4] — 2026-09-23

### Multi-language UI

- Translate the Backups page to English/pt-PT
...
- Add a top-bar language selector with English/Portuguese (pt-PT) UI translations, starting with the shell nav and Login page

## [0.132.1] — 2026-09-23

### Fixed

- A `$` in an app credential or backup password is no longer eaten by Compose's `.env` interpolation

## [0.132.0] — 2026-09-22

### Backups

- Backups page: each Full Backup now has a Restore an app button — pick one application and its data is put back from that snapshot
...
- Auto-create the target directory when applying a WebDAV backup destination, so a fresh subpath (the recommended way to keep Kopia's repository out of a general-purpose WebDAV share) doesn't silently fail with PROPFIND 404

## [0.129.0] — 2026-09-21

### Backups

- Sync Kopia's remote retention policy to the schedule's "keep last" count on every app-data backup
...
- Add a native `webdav` Kopia backup destination alongside disk/SMB/NFS/S3/FTP/SFTP
...
- Push an ntfy alert when a scheduled backup fails, not just an audit-log row

## [0.120.1] — 2026-09-21

### Config

- Nextcloud declares its OnlyOffice dependency as a requires tier
...
- Tailscale Funnel alerts go to ntfy only, no longer also by email

## [0.119.7] — 2026-09-19

### Changed

- Uptime Kuma waits ~7 min before alerting that the Tailscale Funnel is down, so the daily 5-minute DNS blip on its name no longer sends a push
...
- The 6-hourly exposure check now reports exposed apps that aren't running, instead of calling every provisioned hostname healthy.
...
- A stopped (or crashed, or still-starting) app's public address now shows a short "isn't running right now" page instead of a bare 502.
...
- NetBird peers no longer lose their management connection every ~2 minutes: the Job stream, which can't work through Cloudflare, is now declined up front instead of timing out and taking every peer's sync down with it.
...
- Resetting a user's password (or the admin's, in recovery mode) now signs out that user's existing sessions.
...
- MeshCentral re-added for endpoints that can't join the NetBird/Tailscale overlay — Guacamole covers the overlay case, this covers the rest, exposed directly with its own login (no Authelia double-gate)
...
- Kimai admin email/password now re-syncs after first boot, not just at create time

## [0.116.0] — 2026-09-16

### Added

- Home Assistant joins the §480 no-SSO credential fan-out — HA's own OAuth2 login flow + WebSocket admin commands
...
- ITFlow fan-out couldn't read the CSRF token off a modal page — its response body is JSON-wrapped ({"content":...}) even for a plain GET, not raw HTML
...
- Moved DocuSeal from Productivity to the same Files grouping as Stirling-PDF/Samba, on the Home Page and the dashboard
...
- DocuSeal's admin bootstrap now re-syncs the account's login email if it drifts from the Authelia admin's own email

## [0.107.2] — 2026-09-16

### Fixed

- DocuSeal's Home Page tile now links straight to /sign_in instead of the marketing splash at bare root

## [0.107.1] — 2026-09-16

### Changed

- WebDAV no longer gets a Home Page tile — it's a native-client backup destination, not something a person browses to
...
- WebDAV config-panel comment created a phantom required VAR field (the scanner can't tell a comment from a real reference); found and fixed the same landmine pre-existing in Vaultwarden's compose file

## [0.106.1] — 2026-09-16

### Fixed

- WebDAV username/password now show in the config panel (were generated correctly but invisible — not referenced in the compose file)
...
- Access-denied redirect now points at the dashboard's own subdomain, not the bare exposure domain (which serves the Home Page)
...
- Give the header's 5s health poll its own rate limit so it stops crowding out real requests with 429s
...
- Use the Docker host's real LAN IP for lanOnly/overlayOnly access links, not the dashboard's own possibly-public hostname
...
- Replace the pinned/version-pinned badges and Unpin button with a single hover version-info badge; drop the unpin capability entirely
...
- Filter box for the SSO app access grid on the Users page
...
- Users page: SSO app access checkboxes now lay out in a grid instead of ragged flex-wrap
...
- ntfy's subscriber credential is re-issued if the stored password isn't one the dashboard generated, and the write is now verified by reading it back
...
- a generated Authelia config block containing a `$` could splice part of the file into itself, producing a config Authelia refused to load
...
- Uptime Kuma's admin account is now created automatically, clearing its first-run setup wizard
...
- Jellyfin's first-run startup wizard is now completed automatically, so a LAN visitor gets a login instead of an unclaimed server
...
- Configurable Update-page branch (Settings → General → Update branch), so a deployment can track something other than main — e.g. a beta staging host
...
- Homebox — ITFlow is now the single asset/warranty tracker (§301e)
...
- Twenty — open-source CRM (people, companies, pipelines); Postgres + Valkey, exposed direct behind its own login
...
- Self-update: --no-deps on the restart steps so 'up -d frontend' can't recreate the backend mid-deploy (the frontend image now actually gets adopted)
...
- Kimai TRUSTED_HOSTS pipe-separated; self-update forces container recreate so a rebuilt image is actually adopted

## [0.70.0] — 2026-09-10

### Added

- Kimai — time tracking with rates and exports, exposed with its own login (§355)

## [0.69.4] — 2026-09-10

### Fixed

- Self-update now logs the resolved diff + deploy scope and writes a durable pre-build audit breadcrumb, so an under-scoped deploy is diagnosable
