...
export const SERVICES: Record<string, ServiceDefinition> = {
  ...
  'itflow': {
    ...
    // Exposed directly with only its own login (§344/§346): no OIDC (Authelia
    // is OIDC-only; ITFlow does SAML), no way to hide the form, and the client
    // portal must be public — so Authelia can't be the single hidden gate,
    // and stacking it in front is two logins (§342). The dashboard runs
    // ITFlow's setup wizard on start (services/itflowAdminBootstrap.ts) with
    // the Authelia admin's email + this generated password. The §346 first
    // cut stranded a fresh ITFlow by running the wizard before the itfloworg
    // image finished its first-boot DB migration; getSetupState now gates on
    // the `?user` page actually rendering the add_user form, and each step is
    // verified before the next (telemetry — which locks setup — never runs
    // unless the user was created). Turn on 2FA in ITFlow's profile.
