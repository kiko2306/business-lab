...
export const SERVICES: Record<string, ServiceDefinition> = {
  ...
  'docuseal': {
    ...
    // its own login form, so Authelia is the outer forward-auth gate and the
    // user still signs in to DocuSeal with a password. The dashboard runs its
    // first-run /setup wizard (services/docusealAdminBootstrap.ts, §341) with
    // the Authelia admin's email and this generated password, so exposing it
    // lands on a login page rather than a setup form. The password carries in
    // compose only so it renders in the config panel — DocuSeal ignores the
    // env var; read it back from apps/docuseal/.env, or set your own before
    // the first start (docs/app-credentials.md).
