...
export const SERVICES: Record<string, ServiceDefinition> = {
  ...
  'itflow': {
    ...
    // §346 REVERTED: ITFlow should be exposed direct with its own login
    // (`skipAutheliaProtection`) + an auto-run setup wizard, but the
    // itfloworg image runs its first-boot DB migration with no ready signal,
    // so `getSetupState`'s "200 on /setup/" fired before the schema existed —
    // the wizard POSTs hit missing tables (suppressed mysqli errors still
    // "redirect"), and the telemetry step then locked setup
    // (`$config_enable_setup = 0`) with no users table. The bootstrap code
    // (itflowClient.ts / itflowAdminBootstrap.ts) stays, but is not wired
    // until getSetupState is gated on the page actually showing the
    // `add_user` step (schema present). Until then ITFlow stays behind
    // Authelia (two logins). See README.
