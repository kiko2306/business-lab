...
export const SERVICES: Record<string, ServiceDefinition> = {
  ...
  'itflow': {
    ...
    // Exposed directly with only its own login (§344): no OIDC (Authelia is
    // OIDC-only, ITFlow does SAML), no way to hide the form, and the client
    // portal has to be public — so Authelia can't be the single hidden gate,
    ...
    // the Authelia admin's email + this generated password, so there's no
    // manual step and no first-visitor race. Enable 2FA in ITFlow's profile.
