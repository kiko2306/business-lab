...
export const SERVICES: Record<string, ServiceDefinition> = {
  ...
  'metabase': {
    backup: { engine: 'postgres', service: 'metabase-db' },
    name: 'metabase',
    label: 'Metabase',
    description: 'BI dashboards and questions over your databases',
    icon: 'chart',
    category: 'Monitoring & Management',
    composePath: 'apps/metabase/docker-compose.yml',
    healthCheck: {
      enabled: true,
      type: 'http',
    ...
    // Generated on first start: the internal Postgres password (hidden —
    // app <-> DB only) and MB_ENCRYPTION_SECRET_KEY (visible so it can be
    // backed up; it encrypts every connected data source's stored
    // credentials, and losing it means re-entering them all).
    hiddenGeneratedSecrets: ['METABASE_DB_PASSWORD'],
    autoGeneratedSecrets: ['METABASE_ENCRYPTION_KEY'],
    // MB_SITE_URL feeds email links, public-question URLs and SSO redirects,
    // so it has to follow the public hostname once exposure is enabled.
    exposureEnvKeys: {
      url: ['METABASE_SITE_URL'],
    },
  },
  'miniflux': {
  ...
  'postiz': {
    // Postiz's own Postgres holds the durable data (accounts, connected
    // channels, scheduled posts). Temporal's Postgres is workflow-execution
    // state — not backed up here; if lost, Temporal re-initialises and
    // in-flight schedules are re-driven from Postiz's DB on next start.
    backup: { engine: 'postgres', service: 'postiz-postgres' },
    name: 'postiz',
    label: 'Postiz',
    description: 'Social media scheduling',
    icon: 'megaphone',
    category: 'Productivity',
    composePath: 'apps/postiz/docker-compose.yml',
    healthCheck: {
      enabled: true,
      type: 'http',
      // 307 (redirect to /auth) also counts as healthy; the point is that the
      // in-container nginx and the NestJS backend on :3000 are both up (the
      // §257 failure mode was the backend silently never binding).
      url: 'http://localhost:5000/',
      interval: 30000,
      timeout: 5000,
    },
    // MAIN_URL / FRONTEND_URL / NEXT_PUBLIC_BACKEND_URL are all derived from
    // POSTIZ_MAIN_URL in the compose file, so exposure only has to move the
    // one knob to https://<hostname>.
    exposureEnvKeys: {
      url: ['POSTIZ_MAIN_URL'],
    },
    // JWT_SECRET is visible (autoGenerated) — it encrypts the stored
    // social-provider tokens and must be backed up. The two DB passwords are
    // internal-only.
    autoGeneratedSecrets: ['POSTIZ_JWT_SECRET'],
    hiddenGeneratedSecrets: ['POSTIZ_DB_PASSWORD', 'POSTIZ_TEMPORAL_DB_PASSWORD'],
  },
  'homebox': {
    name: 'homebox',
    label: 'Homebox',
