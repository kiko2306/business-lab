...
export const SERVICES: Record<string, ServiceDefinition> = {
  ...
  'crowdsec': {
    ...
    // Shared LAPI key for the bundled Cloudflare bouncer — pre-fill it in the
    // config panel, then copy it into config/cloudflare-bouncer.yaml.
  ...
  'nocodb': {
    ...
    // NC_AUTH_JWT_SECRET has no default — pre-generate it in the config panel
