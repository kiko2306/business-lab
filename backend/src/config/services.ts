...
export const SERVICES: Record<string, ServiceDefinition> = {
  ...
  'kimai': {
    ...
    hiddenGeneratedSecrets: ['KIMAI_DB_PASSWORD'],
  ...
  'homebox': {
    ...
    autoGeneratedSecrets: ['HOMEBOX_OIDC_CLIENT_SECRET'],
    // Toggle in the config panel rather than a hand-edited .env — drop
    // Homebox's own login form once Authelia OIDC is proven working (§270).
    booleanEnvKeys: ['HBOX_OPTIONS_ALLOW_LOCAL_LOGIN'],
    // Log in via Authelia's OIDC provider instead of Homebox's own form
    // (plan.md §270). The dashboard registers the `homebox` client in
    // Authelia on exposure; these env vars (injected at start, not persisted)
    // point Homebox back at it. Homebox's OIDC callback is
    // <hostname>/api/v1/users/login/oidc/callback and it derives the scheme
    // from HBOX_OPTIONS_TRUST_PROXY, so no public-URL var is needed. `groups`
    // is in the scope list because Homebox requests it by default. Local
    // login stays on until an admin sets HBOX_OPTIONS_ALLOW_LOCAL_LOGIN=false
    // after a first successful OIDC sign-in (§216/§217).
    oidcClient: {
      redirectPaths: ['/api/v1/users/login/oidc/callback'],
      secretEnvKey: 'HOMEBOX_OIDC_CLIENT_SECRET',
      scopes: ['openid', 'profile', 'email', 'groups'],
      appEnv: {
        HBOX_OIDC_ENABLED: 'true',
        HBOX_OIDC_ISSUER_URL: '{ISSUER}',
        HBOX_OIDC_CLIENT_ID: '{CLIENT_ID}',
