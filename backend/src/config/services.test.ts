...
describe('oidcClient declarations', () => {
  // autheliaOidcClients.ts reads secretEnvKey from the app's .env and skips
  // the client if it's unset — so the key must also be in autoGeneratedSecrets
  // (or hiddenGeneratedSecrets), or the client silently never registers.
  it('every oidcClient secret is one the dashboard generates', () => {
    for (const [name, service] of Object.entries(SERVICES)) {
      if (!service.oidcClient) continue;
      const generated = [
        ...(service.autoGeneratedSecrets ?? []),
        ...(service.hiddenGeneratedSecrets ?? []),
      ];
      expect(generated, `${name}: ${service.oidcClient.secretEnvKey} is not generated`).toContain(
        service.oidcClient.secretEnvKey
      );
    }
  });

  it('every oidcClient redirect path is absolute', () => {
    for (const [name, service] of Object.entries(SERVICES)) {
      for (const p of service.oidcClient?.redirectPaths ?? []) {
        expect(p.startsWith('/'), `${name}: redirect path ${p} must start with /`).toBe(true);
      }
    }
  });

  // An app that logs in through Authelia OIDC with its own form hidden must
  // NOT also carry skipAutheliaProtection — that would drop the Authelia
  // client it depends on (§342 filter in userAppAccess).
  it('an OIDC app is never also skipAutheliaProtection', () => {
    for (const [name, service] of Object.entries(SERVICES)) {
      if (service.oidcClient) {
        expect(service.skipAutheliaProtection, `${name}`).not.toBe(true);
      }
    }
  });
});
...
describe('autheliaBypassPaths', () => {
  ...
  it("admits the requests native clients actually send, query strings included", () => {
    ...
    // hotel's property agent, same shape as tally's below.
    const hotel = SERVICES.hotel.autheliaBypassPaths ?? [];
    expect(matchesAny(hotel, '/agent/enrol')).toBe(true);
    expect(matchesAny(hotel, '/agent/units?since=2026-01-01')).toBe(true);
    // The admin API stays gated — a bypass swallowing it would publish every
    // guest's passport details.
    expect(matchesAny(hotel, '/api/units')).toBe(false);
