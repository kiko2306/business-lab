...
export interface ServiceDefinition {
  ...
  // Compose env var(s) hidden from the config panel like
  // `hiddenGeneratedSecrets`, but WITHOUT that field's auto-generate-if-blank
  // behavior — for a var that already has a working compose-level default
  // (e.g. a bundled DB's fixed username/database name) where a random value
  // would actively break something (the DB was already initialized with the
  // default, so a random username wouldn't match it) rather than just being
  // dead weight. Nothing to fill in, nothing to show; leave it unset.
  ...
  // This app logs in against Authelia's own OIDC provider instead of showing
  // its own login form on top of Authelia's (plan.md §216/§217/§270). The
  // dashboard registers a matching confidential client in Authelia's
  // `identity_providers.oidc.clients` on every exposure change — see
  // services/autheliaOidcClients.ts. `redirectPaths` are paths on the app's
  // own public hostname (the sync prepends `https://<hostname>`), so this
  // stays domain-agnostic. `secretEnvKey` must also be in
  // `autoGeneratedSecrets` so its value is filled with no user step.
  /**
   * Path regexes on this app's hostname that Authelia must let through
   * *unauthenticated*, emitted as a `bypass` rule ahead of the app's own
   * `one_factor` rule (Authelia is first-match-wins).
   *
   * Only for APIs that native clients hit with their own credentials, where a
   * 401-redirect-to-login-portal is something the client cannot use (plan.md
   * §415/§423). Each entry widens the public surface of a *publicly exposed*
