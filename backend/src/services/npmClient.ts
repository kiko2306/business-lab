...
function buildGrpcAdvancedConfig(forwardHost: string, forwardPort: number): string {
  return [
    ...
    '    }',
    '    # NetBird holds long-lived gRPC streams that sit idle between messages —',
    "    # management's Sync and, critically, signal's bidirectional ConnectStream",
    '    # (idle whenever a peer has nobody to exchange candidates with yet). The',
    "    # default 60s grpc_read_timeout tears those down mid-connection; the",
    '    # client then reconnects in a tight ~60s loop and never finishes peer',
    '    # setup, so no tunnel ever forms. Give the streams room to stay open and',
    '    # keep the socket alive so a genuinely dead peer is still detected.',
