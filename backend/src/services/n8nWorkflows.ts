...
export function buildCrowdsecAlertWorkflow(opts: {
  ...
  const jsCode = [
    ...
    "}",
    "",
    "// Each IP's longest ban in the batch (§545). With escalating bans (§541)",
    "// one scan's later alerts carry longer bans than its first, and the first",
    "// is the one that survives the dedupe below. Noise alerts count too: their",
    "// ban is just as real.",
    "function secs(d) {",
    "  let t = 0;",
    "  String(d || '').replace(/([\\d.]+)(h|ms|m|s)/g, (_, n, u) => {",
    "    t += parseFloat(n) * ({ h: 3600, m: 60, s: 1, ms: 0.001 })[u];",
    "    return '';",
    "  });",
    "  return t;",
    "}",
    ...
    "  const s = (a && a.source) || {};",
    "  const key = s.ip || s.value || 'unknown';",
    "  for (const d of (a && a.decisions) || []) {",
    "    if (d.type === 'ban' && secs(d.duration) > secs(longest[key])) longest[key] = d.duration;",
    "  }",
    "}",
    "",
    ...
    "  const ban = (enforced && longest[ip]) ? ' · banned ' + longest[ip] : '';",
