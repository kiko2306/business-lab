...
function buildNpmBouncerBlock(): string {
  return [
    ...
    '# rest of them, or unbans it under Settings (reachable over the LAN or',
    '# Tailscale, which bypass NPM).',
 ...
 * alerts into bans, 4h for a first offence and longer for repeat offenders), plus the `http_default` notification wired in when
...
function buildProfilesYaml(notify: boolean): string {
  ...
  // Repeat offenders get longer bans (§538, §541): 4h per decision this value
  // already has, plus 4h, capped at a week. Permanent bans were rejected —
  // false positives are real and scanners sit on recycled cloud IPs. The count
...
# Mirrors CrowdSec's upstream default profiles, plus duration_expr (repeat
# offenders get longer bans) and the http_default notification line,
