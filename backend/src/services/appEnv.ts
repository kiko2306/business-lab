...
async function loadStatus(
  ...
  const fields: ServiceEnvField[] = envVars.map((envVar) => {
    ...
    // for password-complexity rules is precise enough that no separate
    // per-service opt-in list is needed.
    const readableGeneratedPassword =
      !hidden && (service?.autoGeneratedSecrets ?? []).includes(envVar.key) && /ADMIN_PASSWORD$/i.test(envVar.key);
