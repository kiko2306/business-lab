...
async function loadStatus(
  ...
  // Hidden, but deliberately NOT in autoGeneratedSecrets — see the doc
  // comment on hiddenEnvKeys in types/index.ts for why a random fill would
  // be actively wrong for these, not just unnecessary.
  const hiddenEnvKeys = service?.hiddenEnvKeys ?? [];
  ...
  const fields: ServiceEnvField[] = envVars.map((envVar) => {
    const hidden = hiddenGeneratedSecrets.includes(envVar.key) || hiddenEnvKeys.includes(envVar.key);
    ...
    // A generated *_ADMIN_PASSWORD is the login for an app with no SSO
    // (ITFlow, DocuSeal, NocoDB, Kimai, Home Assistant, …) — unlike every
    // other secret-shaped key, the human has to be able to read it back, not
    // just set-and-forget it (app-credentials.md documents reading these
    // from the config panel; before this it was write-only like every other
    // secret, so that was never actually true — found live, §381 follow-up).
    // `autoGeneratedSecrets` (never the hidden ones — those stay fully
    // masked) plus the naming convention `generateSecretFor` already uses
    // for password-complexity rules covers every app login.
    //
    // It is not the whole story, though — that was asserted here and ntfy
    // disproved it (§428): its `subscriber` credentials are generated for the
    // human to type into a phone app, and neither key ends in
    // ADMIN_PASSWORD, so both were masked and the feature was unusable
    // without reading .env. Hence `readableSecrets`, an explicit per-service
    // opt-in for exactly that case. Hidden keys are still never revealed.
    const readableGeneratedPassword =
      !hidden &&
      (((service?.autoGeneratedSecrets ?? []).includes(envVar.key) && /ADMIN_PASSWORD$/i.test(envVar.key)) ||
        (service?.readableSecrets ?? []).includes(envVar.key));
    const secret = hidden || (SECRET_KEY_PATTERN.test(envVar.key) && !readableGeneratedPassword);
...
export async function saveServiceEnv(
  ...
  for (const key of service?.hiddenGeneratedSecrets ?? []) {
    if (!currentValues[key] && !pendingUpdates.has(key)) {
      pendingUpdates.set(key, generateSecretFor(key));
    }
  }
...
export async function ensureGeneratedSecrets(serviceName: string): Promise<string[]> {
  ...
  for (const key of keys) {
    if (SECRET_PLACEHOLDERS.has((currentValues[key] ?? '').trim().toLowerCase())) {
      generated[key] = generateSecretFor(key);
    }
  }
