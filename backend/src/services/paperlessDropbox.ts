 ...
 * `consume/` exists before `compose up` (§219, §220, §310) — a subfolder of
 * the shared tree (Samba serves the whole tree, Nextcloud mounts it as
 * External Storage), not the whole tree: Paperless's consumer watches
 * whatever lands here, OCRs/tags it and deletes the original, so "move a
 * file into `to-paperless/`" is the deliberate way to file something, not
 * "everything shared is visible to Paperless."
 *
 ...
 * 0777` sidesteps the ownership question the same way `samba-init`'s
 * `chmod -R o+rwX /storage` does for the shared tree root (§310):
...
export function ensurePaperlessDropbox(appDir: string): void {
  const dir = path.join(appDir, '..', 'nextcloud', 'data', 'shared', 'to-paperless');
  fs.mkdirSync(dir, { recursive: true });
  try {
    fs.chmodSync(dir, 0o777);
  } catch (err) {
    // chmod succeeds only as the dir's owner (or root). Once the dir already
    // exists owned by another uid — e.g. samba-init's container created it —
