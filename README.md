...
**Version 0.95.2** — full history in the [changelog](/CHANGELOG.md).
...
- [ ] **ntfy's alert stream can't be read by the ntfy app** (§415/§423 —
      Vaultwarden's half is done) — the app retries forever against
      `ntfy.<domain>/homelab-alerts/json`, because Authelia answers a native
      client with a 401-to-login-portal it can't use. Alerts still publish
      fine (via the host port, not through Authelia); only reading them on a
      phone is blocked. The `autheliaBypassPaths` mechanism now exists, but
      a bare bypass here would publish the CrowdSec alert stream — attacker
      IPs included — to anyone who guesses the topic, because **ntfy has no
      auth of its own**. So this needs ntfy's `auth-file` + ACL +
      `auth-default-access: deny-all` generated by the dashboard, plus a
      subscriber token to paste into the app, *and* the publish path
      (`ntfyPublishUrl()` in `n8nWorkflows.ts`) given a token at the same
      time — or CrowdSec alerts stop being delivered silently.
