...
**Version 0.44.0** — full history in the [changelog](/CHANGELOG.md).
...
- [ ] **Wire up the "trust the proxy" knobs the §210.2 audit found** (§216,
      ...
      - **OIDC against Authelia's own provider, then disable the local
        ...
        **Vikunja (§271) and Homebox (§272) are wired** — both unproven, see
        the @mat items below. Remaining per app — declare `oidcClient` +
        `<APP>_OIDC_CLIENT_SECRET` in `autoGeneratedSecrets` + the app's
        `oidcClient.appEnv` template, add the compose passthroughs, then
...
- [ ] **@mat: prove Homebox's Authelia OIDC login live** (§270, §272) — the
      ...
      works. If login fails with a PKCE error, add a per-app `require_pkce`
      flag to `renderOidcClientsBlock`.
