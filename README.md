...
- [ ] **OnlyOffice runs with no database at all** (§88.6, §168) — Document
      Server 9.4 dropped the bundled PostgreSQL + RabbitMQ, so there is
      nothing to dump and document state does not actually persist. Add
      `onlyoffice-db` (`postgres:16-alpine`) and `onlyoffice-rabbitmq`
      services to `apps/onlyoffice/docker-compose.yml`, wire `DB_*` / `AMQP_URI`,
      make the passwords `autoGeneratedSecrets`, and add
