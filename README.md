...
**Version 0.43.2** — full history in the [changelog](/CHANGELOG.md).
...
- [ ] **Wire up the "trust the proxy" knobs the §210.2 audit found** (§216,
      ...
      - **OIDC against Authelia's own provider, then disable the local
        ...
        **Vikunja is wired** (§271, unproven — see the @mat item below).
        Remaining per app — declare `oidcClient` + `<APP>_OIDC_CLIENT_SECRET`
        in `autoGeneratedSecrets` + the app's `oidcClient.appEnv` template,
        add the compose passthroughs, then **@mat** proves it live and flips
        the disable-local-login flag: Immich (Admin Settings, OAuth-only, has
        a CLI recovery path — no env path, different shape), Mealie
        (`ALLOW_PASSWORD_LOGIN=false` — one community report of this not
        working on some version, verify live), Homebox
