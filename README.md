...
**Version 0.43.1** — full history in the [changelog](/CHANGELOG.md).
...
- [ ] **Wire up the "trust the proxy" knobs the §210.2 audit found** (§216,
      ...
      - **OIDC against Authelia's own provider, then disable the local
        ...
        exposure change. Remaining per app — declare `oidcClient` +
        `<APP>_OIDC_CLIENT_SECRET` in `autoGeneratedSecrets`, add the app's
        own OIDC env, then **@mat** logs in once via Authelia and flips the
        disable-local-login flag: Immich (Admin Settings, OAuth-only, has a
        CLI recovery path), Vikunja (`VIKUNJA_AUTH_LOCAL_ENABLED=false`),
