...
| **MeshCentral** | For managing endpoints that can't join the NetBird/Tailscale overlay — the reason it's on the public tunnel at all, unlike Guacamole (overlay-only). MeshCentral has no OIDC/SAML in the community edition, so it is **exposed directly, not behind Authelia** (§342) — its own signup wizard is the only gate. Open it privately first and create the first account; `ALLOW_NEW_ACCOUNTS` then locks to disable further signups. No dashboard-generated password to read anywhere. |
