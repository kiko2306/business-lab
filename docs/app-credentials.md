...
The routing peer's setup key (`NETBIRD_ROUTING_PEER_SETUP_KEY`) is generated
and kept fresh automatically once a NetBird Personal Access Token
(`NETBIRD_API_TOKEN`) is entered through this app's config in the Business
Lab dashboard (§405) — that token is the one thing typed by hand, since only
NetBird's own UI (Settings → Personal Access Tokens) can mint it, same
pattern as `TAILSCALE_AUTH_KEY`. Without a token, paste in a setup key
generated by hand from NetBird's Remote Network Access wizard instead. See
`apps/netbird-vpn/.env.example`.

**Neither NetBird value can be made non-expiring** — NetBird's own API caps
both Personal Access Tokens and setup keys at 365 days, hard, with no "never
expires" option (checked against its API docs directly). Pick 365 when
NetBird's UI asks. The routing-peer *setup key* self-heals past that: every
`netbird-vpn` start checks its own `valid` flag and re-mints one if it's
expired or revoked (§405.2/§405.3), no human involved. The **PAT itself**
does not — NetBird's API has no way to mint a PAT without a human already
holding a session in its UI, so when the 365-day cap hits, auto-provisioning
just logs a warning and stops until a fresh one is pasted in. There is
currently no dashboard alert for that; it's a known gap, not yet a TODO
item.
