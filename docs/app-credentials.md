...
| **Paperless-ngx** | `PAPERLESS_ADMIN_USER` | `PAPERLESS_ADMIN_PASSWORD` (generated). Behind Authelia the Paperless login form is skipped — it trusts the `Remote-User` header (`PAPERLESS_ENABLE_HTTP_REMOTE_USER`). LAN-direct access on the app's port still uses the form. A header-authed user that doesn't exist yet is auto-created with **no permissions** (the UI 403s), so the dashboard pre-provisions Authelia users (§692/§693): the admin becomes a Paperless superuser; everyone else joins an "Authelia users" group with ordinary document permissions (no workflows, mail rules, users or settings) — on every Paperless start and after each Authelia user sync. `PAPERLESS_ADMIN_USER` stays the LAN-login superuser. The REST API keeps its own token auth. |
| **Kopia** | `kopia` (fixed) | `KOPIA_SERVER_PASSWORD` (generated) for the web UI / REST API. `KOPIA_PASSWORD` is the repository encryption password — also generated, and the one to keep safe: without it the snapshots can't be read on a rebuild. The backup **destination** (disk / SMB / NFS / S3 / WebDAV / FTP / FTPS / SFTP) is set in the dashboard at Settings → Backup destination. WebDAV is Kopia's own native backend (no bridging layer) — enter the full URL, username and password; point it at this repo's own `apps/webdav` app or any other WebDAV server. For FTP/FTPS/SFTP, enter the server (`host` or `host:port`), the remote directory (`/` = root), and the username/password; Kopia reaches all three through the bundled `rclone`. SFTP is password-auth only and does not verify the host key unless you add `--sftp-known-hosts-file=…` in the extra-flags field. |
...
| **Twenty** | the Authelia admin's email | `TWENTY_ADMIN_PASSWORD` (generated). Twenty's SSO (OIDC/SAML) is an Enterprise-licensed feature not in the community edition, so it is **exposed directly, not behind Authelia** (§342) — this account is the only login. The dashboard claims the workspace-owner account through Twenty's own GraphQL API on first start (§421); read the password in the config panel, or set your own **before** the first start. Self-closing once claimed — Twenty refuses a second workspace in this single-workspace deployment, so there's no equivalent of "lock down signups" to do afterwards. `TWENTY_APP_SECRET` (generated, shown in the config panel) also seeds the field-encryption key — back it up; losing it makes encrypted column values unreadable. |
...
| **WebDAV** | `WEBDAV_USERNAME` (default `admin`) | `WEBDAV_PASSWORD` (generated, shown — copy it into whatever native client points at this app). Not a browser app: it's a WebDAV destination for a native client (a backup tool, a sync app) that authenticates itself over Basic Auth and can't follow a redirect, so it is **exposed directly, not behind Authelia** (`skipAutheliaProtection`) — this one account, over Basic Auth, is the only gate. The dashboard renders both into `data/config.yml` on every start (`backend/src/services/webdavConfig.ts`); changing either in the config panel and restarting rewrites the file. Storage location is also a config-panel field: `WEBDAV_NAS_SERVER`/`SHARE`/`USERNAME`/`PASSWORD` point it at a NAS's SMB share instead of local disk (blank server = local, the default); changing any of them and restarting swaps the storage volume (`backend/src/services/webdavMount.ts`) — existing files stay wherever they already were, nothing migrates automatically. |
...
| **MeshCentral** | For managing endpoints that can't join the NetBird/Tailscale overlay — the reason it's on the public tunnel at all, unlike Guacamole (overlay-only). MeshCentral has no OIDC/SAML in the community edition, so it is **exposed directly, not behind Authelia** (§342) — its own login is the only gate. MeshCentral makes the first account created site admin whatever `ALLOW_NEW_ACCOUNTS` says, so the dashboard creates it on start (`meshcentralAdminBootstrap.ts`): the Authelia admin's username + email, password `MESHCENTRAL_ADMIN_PASSWORD` (auto-generated, readable in the config panel). No further signups after that. |
| **Navidrome** | First visit creates the admin account. |
| **Uptime Kuma** | First visit creates the admin account. |
| **Outline** | Authelia OIDC only — Outline has no local accounts or password form, so it is unusable until the app is exposed (`dependsOn` Authelia). The **first** Authelia user to sign in creates the workspace and becomes its admin; Outline has no group→role mapping, so later users are members and (unless the admin allows their email domain under Settings → Security) need an invite first. `OUTLINE_SECRET_KEY` (generated, shown in the config panel) signs sessions and encrypts stored tokens — back it up; losing it logs everyone out. |
| **Mealie** | Ships with `changeme@example.com` / `MyPassword`. The dashboard rotates that password to a generated `MEALIE_ADMIN_PASSWORD` the first time Mealie is reachable (like Guacamole's `guacadmin`) and uses that account to point Mealie's AI recipe parsing at whichever provider Settings → AI API Keys has active for it (§238, §610). Create your own admin user in Mealie; the seeded one is dashboard-owned. AI parsing goes through that provider's OpenAI-compat endpoint — Anthropic documents its own as test-only, and a badly-structured page can still fail to parse regardless of provider. Once exposed, an Authelia OIDC login appears (the dashboard wires the client automatically — §270/§274). After signing in through it once, flip **Configuration → `ALLOW_PASSWORD_LOGIN`** to false to drop Mealie's own username/password form and leave Authelia as the only gate. |
| **Vikunja** | Register the first account; registration can then be disabled. Once exposed, an "Authelia" OIDC login button appears (the dashboard wires the client automatically — §270). After signing in through it once, flip **Configuration → `VIKUNJA_AUTH_LOCAL_ENABLED`** to false to drop Vikunja's own username/password form and leave Authelia as the only gate. For the mobile/desktop app, enter the server URL shown on the app's card (`App URL` badge, `<host>/api/v1`) — confirmed working live, §606. |
| **n8n** | Owner account created on first visit. |
| **Hotel** | Nothing to log in with. Authelia is the identity (plan.md §629) — the app keeps no accounts of its own, and which properties you see comes from the access mapping an admin sets. Its bundled database password is generated and never needs typing. |
| **Tally** | Nothing to log in with. Authelia is the identity (plan.md §629) — the app keeps no accounts of its own, and which stores you see comes from the access mapping an admin sets. Its bundled database password is generated and never needs typing. |
...
**Vaultwarden**, **n8n** (user-management emails), **Paperless**
