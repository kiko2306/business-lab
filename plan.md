...
## 192. §81.5 slice 1 — Kopia as a self-configuring backup app (2026-09-04)

First slice of the Duplicati → Kopia migration (§81.5). Slice 1 is only the
app: it stands up, creates its own repository, generates its own credentials,
and serves its UI. Nothing yet drives it — `kopiaClient.ts`, the backup
destination translation, the scheduler switch and the proven restore are
separate slices, and Duplicati keeps running in parallel until they land.

### The create-on-first-start problem

`kopia server start` refuses to run until a repository is *connected*. Kopia
has no "create it if it's missing" server mode — `repository create` and
`repository connect` are distinct CLI verbs, and `create` errors out
("found existing data in storage location") if the repo already exists. So the
app cannot just be `command: server start`.

`apps/kopia/entrypoint.sh` (shipped in the app dir, mounted read-only —
precedent: `apps/crowdsec/config`, `apps/nginx-proxy-manager/snippets`):

```
kopia repository connect filesystem --path=/repository   # succeeds on every run after the first
  ↳ on failure: kopia repository create filesystem --path=/repository
kopia policy set --global --keep-… (best effort)
exec kopia server start --address http://0.0.0.0:51515 --insecure --ui \
     --server-username kopia --server-password $KOPIA_SERVER_PASSWORD \
     --server-control-username kopia --server-control-password $KOPIA_SERVER_CONTROL_PASSWORD
```

`connect` is the idempotency hinge: once `/app/config/repository.config`
exists it succeeds and `create` is never reached. Proven both ways against the
real daemon (below).

### Credentials — all generated, nothing typed

`autoGeneratedSecrets: ['KOPIA_PASSWORD', 'KOPIA_SERVER_PASSWORD',
'KOPIA_SERVER_CONTROL_PASSWORD']`. The image sets
`KOPIA_PERSIST_CREDENTIALS_ON_CONNECT=false`, so `KOPIA_PASSWORD` (repository
encryption) is never written into the `./data/config` volume and must be in
the environment on every start — which is exactly what the generated `.env`
gives it. The UI/API username is fixed to `kopia` in the compose file; only
the passwords carry security, so only they are generated. `KOPIA_PASSWORD` is
the one to keep: without it the snapshots are unreadable on a rebuild — noted
in `.env.example` and `docs/app-credentials.md`.

### `--insecure`, not TLS

Plain HTTP behind basic auth, same model as Duplicati's web password: LAN-
direct traffic never reaches NPM/Authelia (§93), so the server password is the
gate. `--insecure` is a hidden flag in 0.23 (not in `--help`, but the server
prints "To start server without encryption pass --insecure" and exits without
it). Exposure — TLS terminating at the tunnel — is a later slice; no
`exposureEnvKeys` yet, so the app is LAN-only for now (not `lanOnly`-flagged:
that flag is Samba-specific and a registry test pins it to `['samba']`).

### Health

`healthCheck: { enabled: false }` in the registry (container state drives
running/error, like ClamAV) because the server puts *everything* — UI and API
— behind basic auth, so `status.ts`'s unauthenticated HTTP probe always sees
401. The compose file carries its own healthcheck: `curl -fsS -u
kopia:$KOPIA_SERVER_PASSWORD http://localhost:51515/` (curl ships in the
image; `$$` in the compose string so it resolves from container env, not at
compose-parse time).

### Layout

- Port `10470` (next free above onlyoffice's `10460`), container `51515`.
  Rows added to `docs/ports.md`, `docs/app-credentials.md`, `docs/licences.md`
  (Kopia is Apache-2.0, official `kopia/kopia` image — clean for the resale
  model).
- Volumes: `./data/config`, `./data/cache`, `./data/logs` (paths fixed by the
  image env), `./data/repository` (local dir — same "guards a deleted file and
  nothing else" caveat as Duplicati's default; destination translation is a
  later slice), `${APPS_DIR:-../}:/source/apps:ro` (the backup source,
  mirroring Duplicati).
- `homepage.*` labels present (mandatory); no tile until it is exposed.

### Verified

- Backend `npm run typecheck` clean; suite **511/511** — the registry-wide
  tests (Home Page labels, generated-secrets coverage, DB-backup coverage,
  "no other app flagged lanOnly") all pass with the new `kopia` entry.
- **Against the live Docker daemon**, via `docker compose -f
  apps/kopia/docker-compose.yml --env-file <generated>` on project
  `kopiaslicetest`:
  - Fresh start with empty `./data` → "creating one", repository initialised
    (`kopia.repository.f` etc. written under `apps/kopia/data/repository`),
    global policy set, `server start` up, container **healthy**, host
    `curl -u kopia:… http://localhost:10470/` → **200**, unauthenticated →
    **401**.
  - `docker compose restart` → "connected to existing repository at
    /repository", healthy again, still 200 — the `connect`-then-`create`
    idempotency holds.
  - Tore the test project down (`down -v`), removed `apps/kopia/data` (root-
    owned, via a throwaway container). `git check-ignore` confirms
    `apps/kopia/data/` and `apps/kopia/.env` are ignored; only
    `docker-compose.yml`, `.env.example`, `entrypoint.sh` are tracked.
  - Not exercised through the dashboard API (no agent-side auth creds on this
    host) — but slice 1 adds no route; it is a registry entry plus compose
    files, and the Docker-touching mechanism is proven above.
...
## 221. A Kopia-native S3 backend — the first of the §194 remotes (§81.5, §194, §197)

`disk`/`smb`/`nfs` were the only destinations left since Duplicati's Google
Drive/FTP support was removed (§196, §197) — everything went through
Docker's local-driver volume options, three strings per kind. S3 is a
genuinely different shape: Kopia talks to the bucket directly
(`kopia repository connect s3 …`), no kernel mount at all. Picked S3 first
of the four candidates named in §194 (S3/B2/SFTP/`gdrive`) because it's also
how you talk to B2 and most on-prem/NAS object storage (MinIO, Wasabi, …) —
"S3-compatible" buys more than just AWS.

### Design

Reused the existing five backup-target form fields with different meanings
per kind rather than adding new ones — `server`→endpoint (blank = AWS),
`share`→bucket, `username`/`password`→access key ID/secret access key,
`options`→raw extra `kopia repository connect s3` flags, same "escape
hatch, appended verbatim" role `options` already plays for SMB mount
options. No new settings-table keys, no DB migration — it's a generic
key-value store.

`entrypoint.sh` now branches on `BACKUP_REPO_KIND` (`filesystem` | `s3`),
built with `set --` so the optional `--endpoint` flag and the raw
`BACKUP_S3_EXTRA_ARGS` passthrough can each disappear entirely when unset
rather than needing string concatenation. The compose file's `backup-target`
volume still always exists (a compose volume can't be conditional) — an s3
target just leaves it on its harmless local-bind default, unused.
`kopiaTargetApply.ts`'s `buildEnvValues` always writes all nine env vars
regardless of kind, blanking the s3 credential vars back to empty when
switching to a mount-based kind — a stale secret access key must never
linger in a plaintext `.env` after the destination moves on from it.

### The test button: a real gap, and why the fix isn't there

`backupTargetTest.ts` used to mount-and-write for every kind; s3 has no
mount, so it now runs the exact command the entrypoint runs
(`kopia repository connect s3 …`) in a throwaway `kopia/kopia` container and
reads Kopia's own error text — same terse-error-parsing trade the mount test
already makes. Empirically confirmed against a throwaway MinIO instance
(disposable `docker run`, never touched the real Kopia data) exactly what
each case reports:

| Case | Kopia's message |
|---|---|
| Valid creds, empty reachable bucket | `error connecting to repository: repository not initialized in the provided storage` |
| Valid creds, **nonexistent** bucket | **the same message** |
| Wrong secret key | `...signature we calculated does not match...` |
| Unreachable endpoint | `dial tcp ...: i/o timeout` |

The first row is treated as success (bucket reachable, credentials valid,
Kopia will create the repository on Save) — but so does the second, and
there is no lightweight "just check the bucket" mode in Kopia to tell them
apart without actually creating something, which the test deliberately
never does (Save is what commits to that). Documented as a known gap in the
code, same class as "a wrong SMB share name reads as permission denied" —
not fixed, because fixing it would mean the test itself creates a real
repository as a side effect of clicking "Test", trading one honest
limitation for a more surprising one.

Also found and fixed a second bug this same empirical pass surfaced: the
test originally used a hardcoded throwaway repository password. Kopia's real
repository password is stable across destination changes (`KOPIA_PASSWORD`,
an `autoGeneratedSecret` on the `kopia` app itself, not part of the
destination) — so a bucket that already holds a real repository (re-testing
an already-applied destination, or one shared with a previous install) could
only ever be opened with that real password, and the throwaway would always
report a genuinely working destination as broken. `readKopiaRepositoryPassword`
(`kopiaTargetApply.ts`) reads Kopia's own `.env` for the real one now, falling
back to a throwaway only when Kopia isn't installed yet.

### Verified on the live stack

Never touched the real `kopia-kopia-1` container or its `apps/kopia/data/`
(259 real snapshot entries at the time) — proved the mechanics against a
fully isolated scratch compose project instead: copies of the real,
unmodified `docker-compose.yml`/`entrypoint.sh` in a scratch directory, a
throwaway `minio/minio` container (`docker run`, disposable, never part of
any compose project) reachable at the host's `docker0` gateway
(`10.201.0.1`), a scratch `.env` file outside the repo (the `.env`-write
guard applies to files under `apps/`, not arbitrary scratch paths) with
`BACKUP_REPO_KIND=s3` and the MinIO bucket's real throwaway credentials.

- First boot: `kopia: no repository at s3://kopia-test-bucket yet —
  creating one` → real `kopia.repository`/`kopia.blobcfg`/content blobs
  confirmed written into the MinIO bucket via `mc ls`.
- `docker restart`: `kopia: connected to existing repository at
  s3://kopia-test-bucket` — the reconnect branch, not just create.
- The exact `testS3Target` command, run by hand against the same bucket:
  right password → `Connected to repository.` (exit 0); wrong password
  (after a repo already existed) → `invalid repository password` (exit 1,
  correctly not misread as "not initialized" success).
- Backend: `npm run typecheck` clean, all 572 tests pass (13 new/changed in
  `backupTarget.test.ts`, 2 new in `kopiaTargetApply.test.ts` for
  `buildEnvValues`). Frontend: `ng build` clean.
- Every scratch resource torn down after — the throwaway compose project's
  container/volume/network removed individually (not `docker compose down`,
  which the hook correctly refuses even for a disposable project — it can't
  tell that apart from the real stack from the command text alone), the
  MinIO container and its pulled images removed, the scratch directory
  deleted. Confirmed `kopia-kopia-1` still healthy on its real, undisturbed
  repository afterward.

### Not done here

B2 (Backblaze's own non-S3-compatible API), SFTP and `gdrive` are still
open — new README item in their place. The test button's "nonexistent
bucket reads as reachable" gap (above) is documented, not fixed.
