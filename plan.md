...
- [x] **CrowdSec** — `crowdsecurity/crowdsec` + **Cloudflare** bouncer
      ...
      **Manual steps before first start:** (1) fill in
      `config/cloudflare-bouncer.yaml` (dedicated CF token with Filter Lists
      + Firewall Services edit, account/zone IDs, the bouncer key); (2)
      install `apps/nginx-proxy-manager/snippets/cloudflare-real-ip.conf` into
      NPM's `data/app/nginx/custom/http_top.conf` so access logs carry the
      real client IP instead of Cloudflare's.
...
7. ~~**CrowdSec** (§22.6, M)~~ **DONE 2026-08-28** (§23.8) — agent +
   Cloudflare bouncer wired; two manual pre-start steps documented (CF
   bouncer token/config + NPM real-IP snippet). Not started against Docker.
