...
- **§0 principle 3 sweep** — the apps added this session (ntfy, stirling-pdf,
  waha, nocodb) still ship `.env.example` secrets as `change-me`
  (`WAHA_API_KEY`, dashboard/swagger passwords, `NOCODB_DB_PASSWORD`). They
  resolve via the UI config panel (principle 2 OK), but the generatable ones
  should move to `autoGeneratedSecrets` / `hiddenGeneratedSecrets` so they
  need no user step at all (principle 3). Audit every app's `.env.example`
  for the same.
