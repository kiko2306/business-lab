#!/usr/bin/env bash
# One-command bootstrap: sets up this host (see setup_server.sh - packages,
# Docker, the daemon address-pool, cloudflared's transport, and the prompted
# fixed-IP/passwordless-sudo steps), then generates the env file with safe
# defaults/secrets if needed, and builds and starts the full stack. Everything
# past this point (the first admin account, per-app secrets, exposure
# settings) is configured from the dashboard itself - see the printed URL at
# the end.
#
